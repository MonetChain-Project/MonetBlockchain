...
public class SparseMatrixPerfITCase {
	...
	//region monetMatrixTestAdapter

	public static class monetMatrixTestAdapter extends MatrixTestAdapter {

		private final Matrix matrix;

		public monetMatrixTestAdapter(final Matrix matrix, final int numEntriesPerRow, final byte[] bytes, final String name) {
	...
	private static MatrixTestAdapterFactory[] createTestAdapterFactories() {
		return new MatrixTestAdapterFactory[] {
				(rows, entriesPerRow, bytes) -> {
					...
					return new monetMatrixTestAdapter(matrix, entriesPerRow, bytes, "TunedSparseMatrix");
				},
				(rows, entriesPerRow, bytes) -> {
					final Matrix matrix = new SparseMatrix(rows, rows, entriesPerRow);
					return new monetMatrixTestAdapter(matrix, entriesPerRow, bytes, "SparseMatrix");
				},
				//				(rows, entriesPerRow, bytes) -> {
				...
				//					return new monetMatrixTestAdapter(matrix, entriesPerRow, bytes, "MtjSparseMatrix");
				//				},
				(rows, entriesPerRow, bytes) -> {
					final CompRowMatrix mtjMatrix = createMtjMatrix(rows, entriesPerRow, bytes);
					final Matrix matrix = new TunedMtjSparseMatrix(rows, rows, mtjMatrix);
					return new monetMatrixTestAdapter(matrix, entriesPerRow, bytes, "TunedMtjSparseMatrix");
				},
