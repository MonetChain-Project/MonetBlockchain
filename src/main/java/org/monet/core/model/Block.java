...
public class Block extends VerifiableEntity {
	...
	public Block(
		...
		super(BlockHeight.ONE.equals(height) ? BlockTypes.monetESIS : BlockTypes.REGULAR, BLOCK_VERSION, timeStamp, harvester);
	...
	public Amount getTotalFee() {
		final long rawTotalFee = this.transactions.stream()
				...
				.map(tx -> tx.getFee().getNumMicromonet())
				.reduce(0L, Long::sum);
		return Amount.fromMicromonet(rawTotalFee);
	}
