...
/**
 * An abstract transaction class that serves as the base class of all monet transactions.
 */
public abstract class Transaction extends VerifiableEntity implements Comparable<Transaction> {
	...
	public Amount getFee() {
		final Address monetesisAddress = NetworkInfos.getDefault().getmonetesisBlockInfo().getAddress();
		if (this.getSigner().getAddress().equals(monetesisAddress)) {
			return Amount.ZERO;
	...
	private Amount getMinimumFee() {
		return monetGlobals.getTransactionFeeCalculator().calculateMinimumFee(this);
	}
}