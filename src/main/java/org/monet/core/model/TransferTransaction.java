...
import org.monet.core.messages.MessageFactory;
import org.monet.core.model.mosaic.*;
import org.monet.core.model.observers.*;
import org.monet.core.model.primitive.*;
import org.monet.core.model.transactions.extensions.*;
import org.monet.core.serialization.*;
import org.monet.core.time.TimeInstant;
...
public class TransferTransaction extends Transaction {
	...
	public Amount getXemTransferAmount() {
		...
		return this.getAttachment().getMosaics().stream()
				...
				.map(p -> Amount.fromMicromonet(this.getRawQuantity(p.getQuantity())))
				.findFirst()
				.orElse(null);
	}

	private long getRawQuantity(final Quantity quantity) {
		return BigInteger.valueOf(this.amount.getNumMicromonet())
				.multiply(BigInteger.valueOf(quantity.getRaw()))
				.divide(BigInteger.valueOf(Amount.MICROmonetS_IN_monet))
				.longValueExact();
	}
	...
	private Notification createMosaicLevyNotification(final MosaicLevy levy) {
		return MosaicConstants.MOSAIC_ID_XEM.equals(levy.getMosaicId())
				? new BalanceTransferNotification(this.getSigner(), levy.getRecipient(), Amount.fromMicromonet(levy.getFee().getRaw()))
