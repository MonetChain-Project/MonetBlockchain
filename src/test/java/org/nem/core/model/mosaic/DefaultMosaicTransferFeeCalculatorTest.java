...
public class DefaultMosaicTransferFeeCalculatorTest {
	...
	public void levyCanBeOneXemForTenMosaicsWhenFeeTypeIsPercentile() {
		...
		final MosaicLevyLookup lookup = id -> new MosaicLevy(
				...
				new Quantity(10000L * Amount.MICROmonetS_IN_monet / 10L));
		...
		// Assert:
		assertMosaicXemLevy(levy200, Amount.frommonet(20));
		assertMosaicXemLevy(levy555, Amount.frommonet(55).add(Amount.fromMicromonet(500000)));
		assertMosaicXemLevy(levy800, Amount.frommonet(80));
	}

	@Test
	public void levyCanBeTenXemForOmonetosaicWhenFeeTypeIsPercentile() {
		// Arrange:
		...
		final MosaicLevyLookup lookup = id -> new MosaicLevy(
				...
				RECIPIENT,
				MosaicConstants.MOSAIC_ID_XEM,
				new Quantity(10000L * Amount.MICROmonetS_IN_monet * 10));
		final MosaicTransferFeeCalculator calculator = new DefaultMosaicTransferFeeCalculator(lookup);
		...
		// Assert:
		assertMosaicXemLevy(levy200, Amount.frommonet(2000));
		assertMosaicXemLevy(levy555, Amount.frommonet(5550));
		assertMosaicXemLevy(levy800, Amount.frommonet(8000));
	}
	...
	private static void assertMosaicXemLevy(final MosaicLevy levy, final Amount expectedFee) {
		...
		Assert.assertThat(Amount.fromMicromonet(levy.getFee().getRaw()), IsEqual.equalTo(expectedFee));
	}
