package org.monet.core.model.mosaic;

import org.hamcrest.core.*;
import org.junit.*;
import org.monet.core.crypto.PublicKey;
import org.monet.core.model.primitive.*;

public class MosaicConstantsTest {
	private static final PublicKey NAMESPACE_OWNER_monet_KEY = PublicKey.fromHexString("3e82e1c1e4a75adaa3cba8c101c3cd31d9817a2eb966eb3b511fb2ed45b8e262");
	private static final Quantity monet_XEM_SUPPLY = Quantity.fromValue(8_999_999_999L);

	// region monet namespace

	@Test
	public void namespaceOwnermonetHasExpectedPublicKey() {
		// Assert:
		Assert.assertThat(MosaicConstants.NAMESPACE_OWNER_monet.getAddress().getPublicKey(), IsEqual.equalTo(NAMESPACE_OWNER_monet_KEY));
	}

	@Test
	public void namespacemonetHasExpectedProperties() {
		// Assert:
		Assert.assertThat(MosaicConstants.NAMESPACE_monet.getId(), IsSame.sameInstance(MosaicConstants.NAMESPACE_ID_monet));
		Assert.assertThat(MosaicConstants.NAMESPACE_monet.getOwner(), IsSame.sameInstance(MosaicConstants.NAMESPACE_OWNER_monet));
		Assert.assertThat(MosaicConstants.NAMESPACE_monet.getHeight(), IsEqual.equalTo(BlockHeight.MAX));
	}

	// endregion

	// region xem mosaic definition

	@Test
	public void mosaicIdXemHasExpectedProperties() {
		// Assert:
		Assert.assertThat(MosaicConstants.MOSAIC_ID_XEM.getNamespaceId(), IsEqual.equalTo(MosaicConstants.NAMESPACE_ID_monet));
		Assert.assertThat(MosaicConstants.MOSAIC_ID_XEM.getName(), IsEqual.equalTo("xem"));
	}

	@Test
	public void mosaicDefinitionXemHasExpectedProperties() {
		// Assert:
		Assert.assertThat(MosaicConstants.MOSAIC_DEFINITION_XEM.getCreator(), IsEqual.equalTo(MosaicConstants.NAMESPACE_OWNER_monet));
		Assert.assertThat(MosaicConstants.MOSAIC_DEFINITION_XEM.getDescriptor(), IsEqual.equalTo(new MosaicDescriptor("reserved xem mosaic")));
		Assert.assertThat(MosaicConstants.MOSAIC_DEFINITION_XEM.getId(), IsEqual.equalTo(MosaicConstants.MOSAIC_ID_XEM));
		Assert.assertThat(MosaicConstants.MOSAIC_DEFINITION_XEM.isMosaicLevyPresent(), IsEqual.equalTo(false));
		Assert.assertThat(MosaicConstants.MOSAIC_DEFINITION_XEM.getMosaicLevy(), IsNull.nullValue());

		final MosaicProperties properties = MosaicConstants.MOSAIC_DEFINITION_XEM.getProperties();
		Assert.assertThat(properties.asCollection().size(), IsEqual.equalTo(4));
		Assert.assertThat(properties.getInitialSupply(), IsEqual.equalTo(monet_XEM_SUPPLY.getRaw()));
		Assert.assertThat(properties.getDivisibility(), IsEqual.equalTo(6));
		Assert.assertThat(properties.isTransferable(), IsEqual.equalTo(true));
		Assert.assertThat(properties.isSupplyMutable(), IsEqual.equalTo(false));
	}

	// endregion
}